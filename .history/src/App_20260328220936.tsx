...
export default function App() {
  ...
  const fetchEmails = async (append: boolean = false) => {
    ...
    try {
      const res = await fetch('/api/emails?maxResults=50');
      if (res.ok) {
        const data = await res.json();
        if (append) {
          setEmails(prev => {
  ...
  useEffect(() => {
    if (emails.length > 0 && emails.length <= 5 && !loadingMore) {
      fetchEmails(true);
    }
  }, [emails.length]);
  ...
  const handleAction = async (emailId: string, action: 'delete' | 'skip', emailData: Email) => {
    // Batch delete mode: accumulate deletes locally, skip still works normally
    if (batchDeleteMode && action === 'delete') {
      ...
      <main className="pt-24 px-6 max-w-lg mx-auto">
        {loading ? (
          ...
          <div className="animate-in fade-in duration-500">
            <section className="mt-4 mb-10">
              ...
              <div className="flex justify-between items-end mb-4">
                ...
                <div className="text-right">
                  <span className="text-primary font-black text-3xl">
                    {emails.length > 0 ? Math.round((20 - emails.length) / 20 * 100) : 100}%
                  </span>
              ...
              <div className="h-1.5 w-full bg-surface-container-highest rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${emails.length > 0 ? Math.round((20 - emails.length) / 20 * 100) : 100}%` }}
                ></div>
              </div>
            </section>
            ...
            <CardStack 
              emails={emails}
              onSwipeLeft={(email) => handleAction(email.id, 'skip', email)}
              onSwipeRight={(email) => handleAction(email.id, 'delete', email)}
            />
          </div>
        ) : activeTab === 'stats' ? (
